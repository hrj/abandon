...
class ComplexProcessTest extends FlatSpec with Matchers with Inside {
	"Abandon" should "handle simple xml test case without configuration" in {
	  ...
	  val xmlBalSettings = XmlExportSettings(BalanceType, None, Seq("not-used.xml"), false)
	  val xmlTxnSettings = XmlExportSettings(JournalType, None, Seq("not-used.xml"), false)
