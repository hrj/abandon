...
object SettingsHelper {
  ...
  def makeSettings(configFileName: String, withVersion: Boolean) = {
    ...
    if (file.exists) {
      try {
        ...
        val exports = exportConfigs.map(makeExportSettings(_, withVersion))
  ...
  def makeExportSettings(config: Config, withVersion: Boolean) = {
    ...
    exportType match {
      case "journal" =>
        ...
        case "xml" =>
            val accountMatch = config.optional("accountMatch") { _.getStringList(_).asScala }
            XmlExportSettings(JournalType, accountMatch, outFiles, withVersion)
        case _ =>
      ...
      case "balance" =>
        ...
        case "xml" =>
            val accountMatch = config.optional("accountMatch") { _.getStringList(_).asScala }
            XmlExportSettings(BalanceType, accountMatch, outFiles, withVersion)
        case _ =>
            val message = s"Found '$exportType', '$exportFormat'; expected 'ledger' or 'xml'."
            throw new ConfigException.BadValue(config.origin, "type", message)
      }
...
case class XmlExportSettings(exportType: OutputType, _accountMatch: Option[Seq[String]], _outFiles: Seq[String], withVersion: Boolean) extends ExportSettings(_accountMatch, _outFiles)
